...
func OpenDatabase(cfg config.Config) (*gorm.DB, error) {
	...
	if !databaseExists || !hasTables {
		...
		// also create alert tables (they are in the alert package, not entities)
		if err := migration.SeedModelPrices(db); err != nil {
			return nil, fmt.Errorf("seed fresh database model prices: %w", err)
		}
		// also create alert tables
		if err := db.AutoMigrate(&alert.AlertChannel{}, &alert.AlertRule{}, &alert.AlertEvent{}); err != nil {
			return nil, fmt.Errorf("auto migrate alert tables: %w", err)
		}
