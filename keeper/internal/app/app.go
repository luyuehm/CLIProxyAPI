...
func NewWithConfig(cfg config.Config) (*App, error) {
	...
	authFilesManagementService := service.NewAuthFilesManagementService(cpaClient)
		...
		costAllocationHandler := costallocation.NewHandler(costAllocationService)
	...
	costBudgetService := costbudget.NewService(db)
	costBudgetHandler := costbudget.NewHandler(costBudgetService)
	...
	if err := userService.SeedAdmin(context.Background(), cfg.UserAdminUsername, cfg.UserAdminPassword, cfg.UserAdminEmail); err != nil {
		logrus.WithError(err).Warn("seed admin user failed")
	}
	userJWTAuth := auth.NewUserJWTAuth(cfg.UserJWTSecret, cfg.UserTokenExpiry, userRepo)
	...
	return &App{
		...
		Router: api.NewRouter(
			...
			api.OptionalProviders{
				...
				CostBudget:     costBudgetHandler,
				CostAllocation: costAllocationHandler,
				ContentFilter:  contentFilterService,
				Routes:         routeConfigService,
				Alert:          alertService,
				CPARequestLog:  cpaClient,
