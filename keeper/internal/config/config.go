...
type Config struct {
	...
	// UserAdminUsername 是 RBAC 用户管理默认管理员用户名（首次启动播种）。
	UserAdminUsername string
	// UserAdminPassword 是 RBAC 用户管理默认管理员密码（首次启动播种）。
	UserAdminPassword string
	// UserAdminEmail 是 RBAC 用户管理默认管理员邮箱（首次启动播种）。
	UserAdminEmail string
	// UserJWTSecret 是用户管理接口 JWT 签名密钥。
	UserJWTSecret string
	// UserTokenExpiry 是用户管理接口 JWT 有效期。
