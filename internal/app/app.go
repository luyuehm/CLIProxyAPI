...
func NewWithConfig(cfg config.Config) (*App, error) {
	...
	// RIC-293 RBAC：用户管理接口使用独立 JWT，与现有 session 登录并存；首次启动播种默认管理员。
	userRepository := repository.NewUserRepository(db)
	if err := userRepository.SeedAdmin(cfg.UserAdminUsername, cfg.UserAdminPassword, cfg.UserAdminEmail); err != nil {
		logrus.WithError(err).Warn("failed to seed default RBAC admin user")
	}
	userJWTAuth := auth.NewUserJWTAuth(cfg.UserJWTSecret, cfg.UserTokenExpiry, userRepository)
	...
	budgetService := costbudget.NewService(db)
	costAllocationService := costallocation.NewService(db)
	routeConfigService := service.NewRouteConfigService(db)
	alertService := alert.NewService(db, cfg.RequestTimeout)
	...
	return &App{
		...
		Router: api.NewRouter(
			...
			api.OptionalProviders{
				...
				AuditLogs:     api.AuditLogsProvider{Usage: usageService, Detail: cpaClient, Enabled: true},
				Users:         userService,
				UserAuth:      userJWTAuth,
				ContentFilter: contentFilterService,
				Routes:        routeConfigService,
				CostBudget:    costbudget.NewHandler(budgetService),
				CostAllocation: costallocation.NewHandler(costAllocationService),
				Alert:         alertService,
			},
		),
	}, nil
